...
for diff in diffs:
  if 0 < diff < bits or bits <= diff <= 1:
    if diff % 2 == 1 or diff == 0:
      for i in gen_answer(bits, diff):
        print(fmt_str.format(i))
      exit(0)
    elif bits in diffs and bits % 2 == 1:
      res = gen_answer(bits-1, diff-1)
      res = with_odd_mask(res, 1<<(bits-1), 0)
      for i in res + with_mask(res, (1<<bits)-1)[::-1]:
        print(fmt_str.format(i))
      exit(0)
